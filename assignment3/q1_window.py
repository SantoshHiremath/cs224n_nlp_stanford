...
def make_windowed_data(data, start, end, window_size = 1):
	...
	### YOUR CODE HERE (5-20 lines)
        ...
        # window data for each word
        for i in range(len(labels)):
            context = padded_sentence[i:i+(2*window_size + 1)]
            words = []
            for word in context:
            	words += word
...
class WindowModel(NERModel):
	...
	n_classes = self.config.n_classes
        ...
        # init vriables
	W = tf.get_variable("W", shape = (embed_size, hidden_size), initializer = tf.contrib.layers.xavier_initializer())
	U = tf.get_variable('U', shape=(hidden_size, n_classes), initializer = tf.contrib.layers.xavier_initializer())
	b1 = tf.get_variable("b1", (hidden_size,), initializer = tf.contrib.layers.xavier_initializer())
	b2 = tf.get_variable("b2", (n_classes,), initializer = tf.contrib.layers.xavier_initializer())
	...
	# using tf.layers
        ...
        h = tf.layers.dense(x, self.config.hidden_size, activation=tf.nn.relu, kernel_initializer=init_var)
        h_drop = tf.layers.dropout(h, dropout_rate)
        pred = tf.layers.dense(h_drop, self.config.n_classes, kernel_initializer=init_var)
