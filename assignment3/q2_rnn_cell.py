...
class RNNCell(tf.contrib.rnn.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope):
            ...
            W_x = tf.get_variable('W_x', shape=(self.input_size, self.output_size), initializer=init_var, dtype=tf.float32)
            W_h = tf.get_variable('W_h', shape=(self.output_size, self.output_size), initializer=init_var, dtype=tf.float32)
            b = tf.get_variable('b', shape=(self.output_size,), dtype=tf.float32, initializer=tf.zeros_initializer())
