...
class GRUCell(tf.contrib.rnn.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope):
            ...
            W_r = tf.get_variable('W_r', shape=(self.input_size, self.state_size), initializer=init_var, dtype=tf.float32)
            U_r = tf.get_variable('U_r', shape=(self.state_size, self.state_size), initializer=init_var, dtype=tf.float32)            
            b_r = tf.get_variable('b_r', shape=(self.state_size,), dtype=tf.float32, initializer=tf.zeros_initializer())
            
            W_z = tf.get_variable('W_z', shape=(self.input_size, self.state_size), initializer=init_var, dtype=tf.float32)
            U_z = tf.get_variable('U_z', shape=(self.state_size, self.state_size), initializer=init_var, dtype=tf.float32)            
            b_z = tf.get_variable('b_z', shape=(self.state_size,), dtype=tf.float32, initializer=tf.zeros_initializer())
            
            W_o = tf.get_variable('W_o', shape=(self.input_size, self.state_size), initializer=init_var, dtype=tf.float32)
            U_o = tf.get_variable('U_o', shape=(self.state_size, self.state_size), initializer=init_var, dtype=tf.float32)            
