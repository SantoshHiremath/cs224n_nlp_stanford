...
def softmaxCostAndGradient(predicted, target, outputVectors, dataset):
    ...
    y[o] = 1    
    J = -np.log(yhat[o]) # cost = sum(-y*log(yhat))
...
def negSamplingCostAndGradient(predicted, target, outputVectors, dataset,
    ...
    # same as J = -np.log(sigmoid(h[target])) - np.sum(np.log(sigmoid(-h[indices[1:]])))
    factors = [-1]*len(indices)
    factors[0] = 1
    
    h = np.dot(U, vc)
    sig = sigmoid(factors * h)    
    J = np.sum(-np.log(sig)) 
...
def cbow(currentWord, C, contextWords, tokens, inputVectors, outputVectors,
    ...
    for word in contextWords:
        idx = tokens[word]
        vc_i = inputVectors[idx]
        _cost, _gradPred, _grad = word2vecCostAndGradient(vc_i, o, U, dataset)
        
        cost += _cost
        gradIn[idx] += _gradPred
        gradOut += _grad
